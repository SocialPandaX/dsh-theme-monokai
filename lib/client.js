...
const SERVER_ENDPOINT = "/dsh-theme-monokai/preference";
...
// server persistence is optional; local fallback still works
}
}

function loadServerTheme(ctx) {
fetch(SERVER_ENDPOINT)
...
if (serverTheme === THEME_ID && snapshot.preference !== THEME_ID) {
ctx.theme.setTheme(THEME_ID);
} else if (serverTheme !== THEME_ID && serverTheme !== "default" && snapshot.preference !== serverTheme) {
ctx.theme.setTheme(serverTheme);
}
})
.catch(() => {});
}
