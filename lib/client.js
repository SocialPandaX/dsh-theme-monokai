...
// The theme service keeps the built-in preference (light/dark/system) in the
// host settings document and adopts it asynchronously on every load or
// document re-sync, re-emitting theme/change after this plugin has already
// asserted Monokai. Re-asserting Monokai inside that same emit races with the
// layout presenter's theme/change listener: when the presenter runs after
// this listener it applies the stale built-in snapshot last, leaving the
// token variables on the default palette while the preference (and the Shiki
// gate) already say Monokai. Deferring the re-assertion out of the current
// emit guarantees the presenter always applies the final Monokai snapshot.
...
ctx.on("theme/change", queueReassert);
queueReassert();
